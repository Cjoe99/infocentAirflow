...
def content_extract_emotion():
    ...
    def process_all_collections(collections):
        """
        處理所有collections並生成結果DataFrame
        """
        processed_links = {}  # 用於追蹤所有collection中的連結
    ...
    if __name__ == "__main__":
        ...
        if connection and cursor:
            try:
                # 將兩個 collection 放入列表中
                collections = [collection1, collection2]

                # 處理所有 collection 並取得結果
                result_df = process_all_collections(collections)
...
with DAG(
    ...
    description='content extract emotion',
    schedule_interval=timedelta(days=1),  # 每天運行一次
    catchup=False,
) as dag:

    run_selenium_task = PythonOperator(
        task_id='run_duration',
        python_callable= content_extract_emotion
    )